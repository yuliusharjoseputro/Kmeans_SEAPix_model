...
def process_kmeans_and_show_distribution_using_HSV_not_pixel(directory_path, image_A, image_B, dataColor, k_value, folder_name, t_hue, t_saturation, t_values, mask_color = False):
    ...
    for cluster_num in range(len(sorted_indices)):
        ...
        if mask_color == False:
            ...
            # Save the standard mask
            cv2.imwrite(f'{directory_path}\\A_{folder_name}_B{num_image}.png', standard_mask)
        # else:
            #background black
            color_mask = np.ones((*segmented_image.shape, 3), dtype=np.uint8) * 255
            color_mask[segmented_image == sorted_indices[cluster_num]] = average_colors[cluster_num].astype(np.uint8)
            ...
            # Save the color mask
            color_mask = cv2.cvtColor(color_mask, cv2.COLOR_RGB2BGR)  # Convert back to BGR for saving as an image
            cv2.imwrite(f'{directory_path}\\A_{folder_name}_A{num_image}.png', color_mask)
...
if __name__ == "__main__":
    ...
    # generated_directory = "D:\\JupyterLab\\Program\\colorSeparation\\sample_test_dataset\\"
    # generated_directory = f"D:\\JupyterLab\\Demo\\pix2pixHD\\results\\trainA2trainB_1024p_aug_new_DS\\test_{epoch}\\images"
    # source_directory = "C:\\Baseline_Dataset\\dataset_fix\\new_data_80_20_sharp_color_fold_1024_aug\\test_A\\"
    # directory_path = f"D:\\JupyterLab\\Demo\\pix2pixHD\\results\\trainA2trainB_1024p_aug_new_DS\\test_{epoch}\\"
    # directory_path = "D:\\JupyterLab\\Program\\colorSeparation\\mask_generated\\"
    
    # generated_directory = f"D:\\JupyterLab\\Demo\\pix2pixHD\\results\\trainA2trainB_1024p_no_aug_spatial_atention\\{method}\\test_{epoch}\\images"
    # source_directory = "C:\\Baseline_Dataset\\dataset_fix\\new_data_80_20_sharp_color_fold_1024_no_aug\\test_A\\"
    # directory_path = f"D:\\JupyterLab\\Demo\\pix2pixHD\\results\\trainA2trainB_1024p_no_aug_spatial_atention\\{method}\\test_{epoch}\\"

    phase = "test"
    model = "trainA2trainB_1024p_no_aug_scen_1_newDS"
    # model = "trainA2trainB_1024p_no_aug_spatial_attention"
    # model = "trainA2trainB_1024p_no_aug_batchnorm"
    ...
    for epoch in epoch_list:
        ...
        generated_directory = f"D:\\JupyterLab\\Demo\\pix2pixHD\\results\\{model}\\{method}\\{phase}_{epoch}\\images"
        source_directory = f"C:\\Baseline_Dataset\\dataset_fix\\new_data_80_20_sharp_color_fold_1024_no_aug_scenario1_newDS\\{phase}_A\\"
        source_directory_to_get_init_color = f"C:\\Baseline_Dataset\\dataset_fix\\new_data_80_20_sharp_color_fold_1024_no_aug_scenario1_newDS\\{phase}_A\\"
        directory_path = f"D:\\JupyterLab\\Demo\\pix2pixHD\\results\\{model}\\{method}\\{phase}_{epoch}\\"
        ...
        create_dir(directory_path+"mask_generated_2")
        directory_path = directory_path+"mask_generated_2\\"
        
        #activate these code when you need masks from the original.
        # directory_path_ori_A = f"D:\\JupyterLab\\Demo\\pix2pixHD\\results\\trainA2trainB_1024p_no_aug_spatial_attention\\{method}\\"
        # create_dir(directory_path_ori_A+f"mask_{phase}_original_A")
        # directory_path_ori_A = directory_path_ori_A+f"mask_{phase}_original_A\\"
        # create_dir(directory_path_ori_A+f"mask_{phase}_original_B_check")
        # directory_path_ori_A = directory_path_ori_A+f"mask_{phase}_original_B_check\\"

        print(epoch)
        # print(directory_path)
        # print(directory_path_ori_A)

        # similarity_threshold = 0.27

        for filename_B in os.listdir(generated_directory):
            pattern = r'.*input_label\.png$'  # Adjusted regex pattern to match 'synthesized_image'
            ...
            if match:
                ...
                if not os.path.exists(full_path):
                    ...
                    image_B_gen = cv2.imdecode(np.fromfile(os.path.join(source_directory, "A_"+ new_filename_A + ".png"), dtype=np.uint8), cv2.IMREAD_UNCHANGED)
                    ...
                    # dataColor = detectColor(image_A, threshold)
                    # dataColor = detectColorMajority(image_A_ori, threshold)
                    # dataColor = detectColorWithHSV(image_A_ori, t_hue, t_saturation, t_values)
                    # print(dataColor)
                    ...
                    # image_A_modified, similar_color_distances, dataColor = replace_similar_with_mean_show_distances_and_unique_optimize(image_A_hsv, similarity_threshold)
                    print(folder_name)
                    ...
                    # create_dir(directory_path_ori_A+folder_name)
                    # process_kmeans(image_A, image_B, dataColor, k_value, folder_name, False)

                    if method == "kmeans":
                        # process_kmeans_and_show_distribution(directory_path, image_A_ori, image_B_gen, dataColor, k_value, folder_name, False)
                        # process_mean_shift_and_show_distribution_using_HSV(directory_path, image_A_ori, image_B_gen, dataColor, k_value, folder_name, t_hue, t_saturation, t_values, False)
                        # process_kmeans_and_show_distribution_using_HSV(directory_path, image_A_ori, image_B_gen, dataColor, k_value, folder_name, t_hue, t_saturation, t_values, False)
                        # process_kmeans_and_show_distribution_using_HSV_original_not_pixel(directory_path_ori_A, image_A_hsv, image_B_gen, init_centroid, k_value, folder_name, t_hue, t_saturation, t_values, False)
                        process_kmeans_and_show_distribution_using_HSV_not_pixel(directory_path, image_A_ori, image_B_hsv, init_centroid, k_value, folder_name, t_hue, t_saturation, t_values, False)
                        # process_kmeans_and_show_distribution_using_HSV_original(directory_path_ori_A, image_A_ori, image_B_gen, dataColor, k_value, folder_name, t_hue, t_saturation, t_values, False)
                    elif method == "dbscan":
                    ...
                    # multiply_images_in_directory(os.path.join(directory_path, folder_name))
                else:
