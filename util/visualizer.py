    ...
    def display_current_results(self, visuals, epoch, step):
        if self.tf_log: # show images in tensorboard output
            ...
            for label, image_numpy in visuals.items():
                ...
                image = self.tf.image.decode_png(s.getvalue(), channels=3)
                # Add the batch dimension
                image = self.tf.expand_dims(image, 0)
                # img_sum = self.tf.summary.image("image", s.getvalue(), step=epoch)
                img_sum = self.tf.summary.image("Training data", image, step=epoch)
                # Create a Summary value
                # img_summaries.append(self.tf.compat.v1.Summary.Value(tag=label, image=img_sum))
                # summary = self.tf.compat.v1.Summary(value=[self.tf.compat.v1.Summary.Value(tag=label, image=img_sum)])
                with self.writer.as_default():
                    self.tf.summary.scalar(label, img_sum, step=step)
                    self.writer.flush()
            ...
            # with self.writer.as_default():
            #     self.tf.summary.scalar(value=summary, step=step)
            #     self.writer.flush()
